...
for t in targets:
    with open(t, 'r', encoding='utf-8') as f:
        content = f.read()
    if 'setupSiteSearch' not in content and '</body>' in content:
        new_content = content.replace('</body>', inline_js + '\n</body>')
        with open(t, 'w', encoding='utf-8') as f:
            f.write(new_content)

print("Inserted inline setupSiteSearch script directly into all index files!")
